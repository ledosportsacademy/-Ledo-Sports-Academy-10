...
function initializeApp() {
  ...
  .catch(error => {
    ...
    showMessage('Failed to load data. Using sample data instead.', 'warning');
    ...
    .then(data => {
      ...
      switch(endpoint) {
        ...
        case 'members':
          ...
          break;
        case 'donations':
          appData.donations = data;
          break;
        case 'expenses':
          appData.expenses = data;
          break;
        case 'experiences':
          appData.experiences = data;
          break;
        case 'weekly-fees':
...
function saveActivity() {
  ...
  const activityData = { title, date, time, description, image, status };
  
  // Show loading message
  showMessage('Saving activity...', 'info');
  
  if (currentEditingItem) {
    // Update existing activity
    fetch(`http://localhost:3000/api/activities/${currentEditingItem}`, {
      method: 'PUT',
      headers: {
    ...
    .catch(error => {
      ...
      showMessage('Failed to create activity: ' + error.message, 'error');
    });
  }
}
...
function saveMember() {
  ...
  // Show loading message
  showMessage('Saving member...', 'info');

  if (currentEditingItem) {
    // Update existing member
    fetch(`http://localhost:3000/api/members/${currentEditingItem}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    .then(updatedMember => {
      ...
      if (feeIndex !== -1) {
        ...
        // Update weekly fee record in API
        return fetch(`http://localhost:3000/api/weekly-fees/${currentEditingItem}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({
            memberName: name,
            payments: appData.weeklyFees[feeIndex].payments
          })
        });
      }
    })
    .then(response => {
      if (response && !response.ok) {
        throw new Error('Failed to update weekly fees');
      }
    ...
    .then(newMember => {
      ...
      // Add weekly fee record to local data
      appData.weeklyFees.push(weeklyFeeData);
      
      // Create weekly fee record in API
      return fetch('http://localhost:3000/api/weekly-fees', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    .then(response => {
      if (response && !response.ok) {
        throw new Error('Failed to create weekly fees');
      }
    ...
    .catch(error => {
      ...
      showMessage('Failed to create member: ' + error.message, 'error');
    });
  }
}
...
function saveDonation() {
  ...
  // Prepare data for API
  const donationData = { donorName, amount, date, purpose };
  
  // Show loading message
  showMessage('Saving donation...', 'info');

  if (currentEditingItem) {
    // Update existing donation
    fetch(`http://localhost:3000/api/donations/${currentEditingItem}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    .catch(error => {
      ...
      showMessage('Failed to create donation: ' + error.message, 'error');
    });
  }
}
...
function deleteDonation(id) {
  if (confirm('Are you sure you want to delete this donation record?')) {
    // Show loading message
    showMessage('Deleting donation...', 'info');
...
function saveExpense() {
  ...
  // Prepare data for API
  const expenseData = { description, amount, date, category, vendor, paymentMethod };
  
  // Show loading message
  showMessage('Saving expense...', 'info');

  if (currentEditingItem) {
    // Update existing expense
    fetch(`http://localhost:3000/api/expenses/${currentEditingItem}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    .catch(error => {
      ...
      showMessage('Failed to create expense: ' + error.message, 'error');
    });
  }
}
...
function saveExperience() {
  ...
  // Prepare data for API
  const experienceData = { title, date, description, image };
  
  // Show loading message
  showMessage('Saving experience...', 'info');

  if (currentEditingItem) {
    // Update existing experience
    fetch(`http://localhost:3000/api/experiences/${currentEditingItem}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    .catch(error => {
      ...
      showMessage('Failed to create experience: ' + error.message, 'error');
    });
  }
}
...
function deleteExperience(id) {
  if (confirm('Are you sure you want to delete this experience?')) {
    // Show loading message
    showMessage('Deleting experience...', 'info');
...
function togglePaymentStatus(memberId, paymentDate) {
  ...
  .then(updatedFee => {
    // Update local data
    const feeIndex = appData.weeklyFees.findIndex(function(f) { return f.memberId === memberId; });
    if (feeIndex !== -1) {
      appData.weeklyFees[feeIndex] = updatedFee;
    }
